...
router.get('/reset-password', function(req, res, next) {
    if (req.body.e_mail === undefined || req.body.e_mail == "") {
    ...
    var q = "SELECT id, name, e_mail\n" +
            "  FROM users\n" +
            " WHERE login = $1";
    db.query(q, [ req.params.e_mail ], function (err, result, done) {
        if (err) {
            err.status = 500;
            return next(err);
        }
        ...
        db.query(q, [ result.rows[0].e_mail, id, result.rows[0].id ],
                function (err, result, done)
        {
            if (err) {
                err.status = 500;
            ...
            mail.sendMail({
                from: "Garry Williams <gtwilliams@gmail.com>",
                to: result.rows[0].e_mail,
                subject: "Reset Tour DBAA Web Site Password",
                text: "Dear " + result.rows[0].name +
...
router.post('/reset-password', function(req, res, next) {
    ...
    db.query(q, [ req.body.id ], function (err, result, done) {
        ...
        bcrypt.hash(req.body.password, 8, function (err, hash) {
            ...
            db.query(q, [ hash, result.rows[0].login ],
                    function (err, result, done)
            {
                if (err) {
                    console.log("Error trying to insert new login ID");
